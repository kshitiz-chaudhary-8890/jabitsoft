...
export default function ServiceCardCarousel() {
  ...
  useEffect(() => {
    ...
    const update = () => setStageWidth(el.clientWidth || 1280);
    update();
  ...
  const metrics = useMemo(() => {
    const previewWidth = Math.max(50, Math.min(72, stageWidth * 0.055));
    const gap = Math.max(8, Math.min(12, stageWidth * 0.009));

    /*
     * Keep the real ServiceCard large enough to preserve its locked design.
     * First card starts around the center.
     * As we move forward, the active card gradually shifts right so previous
     * service previews can accumulate on the left.
     */
    const activeWidth = Math.max(
      720,
      Math.min(980, stageWidth * 0.72),
    );

    const startX = Math.max(0, (stageWidth - activeWidth) / 2);
    const endX = Math.max(startX, stageWidth - activeWidth);
    const progress =
      services.length > 1 ? active / (services.length - 1) : 0;

    const activeX = startX + (endX - startX) * progress;
  ...
  const panelState = (index) => {
    ...
    if (index < active) {
      return {
        x: activeX - (active - index) * slot,
        width: activeWidth,
        clipPath: `inset(0 ${inactiveCrop}px 0 0 round 25px)`,
        opacity: Math.max(0.48, 0.72 - (active - index - 1) * 0.05),
        zIndex: 20 - (active - index),
      };
    }
    ...
    return {
      ...
      width: activeWidth,
      clipPath: `inset(0 ${inactiveCrop}px 0 0 round 25px)`,
      opacity: Math.max(0.48, 0.72 - (index - active - 1) * 0.05),
      zIndex: 20 - (index - active),
    };
  };
    ...
    >
      ...
      <div className="services-carousel__stage">
        <div className="services-carousel__viewport">
          <div
            className="services-carousel__cards"
            ref={cardsRef}
          ...
          >
            {services.map((service, index) => {
              const state = panelState(index);
              const isActive = index === active;

              return (
                <div
                  className={`services-carousel__panel ${
                    isActive ? "is-active" : "is-preview"
                  }`}
                  key={service.title}
                  style={{
                    "--panel-x": `${state.x}px`,
                    "--panel-width": `${state.width}px`,
                    "--panel-opacity": state.opacity,
                    "--panel-z": state.zIndex,
                    "--panel-clip": state.clipPath,
                  }}
                  onClick={() => {
        ...
        <div className="services-carousel__controls">
          <button
            type="button"
            className="services-carousel__nav"
            onClick={goPrev}
            disabled={active === 0}
      ...
      <style>{`
        ...
        .services-carousel .service-subhead {
          margin-top: 14px;
        }
        ...
        .services-carousel__panel {
          ...
          transition:
            ...
            clip-path 780ms cubic-bezier(.2,.8,.2,1),
            opacity 420ms ease;
        }

        .services-carousel__panel.is-preview {
          cursor: pointer;
        }

        .services-carousel__panel.is-preview:hover {
          opacity: .9;
        }

        .services-carousel__panel > .svc-card {
          width: 100%;
          max-width: none;
          margin: 0;
          pointer-events: none;
        ...
        .services-carousel__panel.is-active > .svc-card {
          pointer-events: auto;
        }
        ...
        .services-carousel__counter span {
          margin: 0 5px;
          color: #aaa;
        }
        ...
        @media (max-width: 900px) {
          ...
          .services-carousel__panel.is-active {
            position: relative;
            display: block;
            width: 100% !important;
            height: auto;
