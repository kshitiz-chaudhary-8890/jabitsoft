...
const PROBLEMS: Problem[] = [
  {
    ...
    capability: "Agent architecture, tool integration, guardrails and human-in-the-loop review.",
    summary:
  ...
  },
  {
    ...
    trend: {
      ...
      caption: "Illustrative view — release throughput as automation and test coverage land.",
    },
    outcome:
  ...
  },
  {
    ...
    trend: {
      ...
      caption: "Illustrative view — completion rate of the primary journey across release cycles.",
    },
    outcome:
  ...
  },
  {
    ...
    capability: "Process modelling, ERP configuration, systems integration and data migration.",
    summary:
  ...
  },
  {
    ...
    capability: "Technical SEO, intent mapping, content architecture and pipeline measurement.",
    summary:
  ...
  },
  {
    ...
    capability: "Conversion-focused design, performance engineering and CMS enablement.",
    summary:
...
const ICON_PATHS: Record<ServiceKey, string[]> = {
  ...
  erp: ["M4 5h6v5H4zM14 5h6v5h-6zM4 14h6v5H4zM14 14h6v5h-6z", "M10 7.5h4M7 10v4M17 10v4M10 16.5h4"],
  seo: [
...
function SignatureDial({ value, fill, play }: { value: string; fill: number; play: boolean }) {
  const ticks = useMemo(
    () =>
  ...
  return (
    <div className={styles.dial}>
      ...
      >
        ...
        />
        <circle className={styles.dialCore} cx={DIAL_C} cy={DIAL_C} r={DIAL_R - 16} />
      </svg>
          ...
          {i < flow.length - 1 ? (
            <div className={styles.pipeLink} aria-hidden="true">
              ...
              >
                ...
                <path data-pipe-flow="" className={styles.pipeFlow} d="M0 7 H100" />
              </svg>
    ...
    <figure className={styles.chart} data-chart="">
      <div className={styles.chartHead}>
        <span className={styles.chartRange}>
          {trend.points[0]}
          <ArrowIcon size={15} />
          <span className={styles.chartRangeEnd}>{trend.points[trend.points.length - 1]}</span>
        </span>
      ...
      >
        <defs>
          <linearGradient id={areaId} x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor="rgba(20,20,20,0.12)" />
        ...
        {geo.pts.map((point, i) => (
          <circle
            key={`${point.x}-${point.y}`}
            data-chart-dot=""
            className={`${styles.chartDot} ${i === geo.pts.length - 1 ? styles.chartDotLast : ""}`}
            cx={roundSvg(point.x)}
            cy={roundSvg(point.y)}
...
function ProblemDialog({ problem, index, total, onClose, onNavigate }: ProblemDialogProps) {
  const reduced = useReducedMotion();
  ...
  useLayoutEffect(() => {
    ...
    tl.fromTo(scrim, { opacity: 0 }, { opacity: 1, duration: 0.38, ease: "power2.out" }).fromTo(
      dialog,
  ...
  useEffect(() => {
    ...
    const reveal = (block: HTMLElement) => {
      ...
      if (nodes.length) {
        ...
        block.querySelectorAll<SVGPathElement>("[data-pipe-flow]").forEach((flow) => {
          tweens.push(
            gsap.to(flow, {
              strokeDashoffset: -24,
              duration: 1.6,
        ...
        block.querySelectorAll<HTMLElement>("[data-pipe-packet]").forEach((packet, i) => {
          tweens.push(
            gsap.fromTo(
              packet,
              { left: "0%", opacity: 0 },
  ...
  return createPortal(
    <div
      ...
      data-lenis-prevent=""
      onPointerDown={(event) => {
        if (event.target === overlayRef.current || event.target === scrimRef.current) {
          requestClose();
        }
      }}
    >
      ...
      >
        ...
        <div ref={scrollRef} className={styles.dialogScroll} tabIndex={-1} data-lenis-prevent="">
          <div className={styles.dialogGrid}>
            ...
            <div className={styles.rail}>
              <div className={styles.railSticky}>
                ...
                <h3 id={`${dialogId}-title`} className={styles.railTitle} data-intro="">
                  {problem.title}
                </h3>
                ...
                <div className={styles.railDial} data-intro="">
                  ...
                  <p className={styles.railDialLabel}>
                    <span className={styles.railDialTag}>Headline target · illustrative</span>
            ...
            <div className={styles.body}>
                  ...
                  {problem.metrics.map((metric, i) => (
                    <div key={metric.label} className={styles.metric}>
                      ...
                      <span className={styles.metricValue}>
                        <Counter value={metric.value} play={countersOn && !reduced} />
                      </span>
                  ...
                  <div className={styles.column}>
                    ...
                    <ul className={styles.list}>
                      {problem.signals.map((signal) => (
                        <li key={signal} className={styles.listItem} data-stagger="">
              ...
              >
                ...
                <div className={styles.serviceNote}>
                  ...
                  <div>
                    <span className={styles.serviceNoteLabel}>Relevant service</span>
                    <span className={styles.serviceNoteValue}>{problem.serviceFull}</span>
                    <span className={styles.serviceNoteText}>{problem.capability}</span>
                  </div>
                </div>
              </section>
            </div>
...
export default function ProblemsWeSolve() {
  ...
  const navigate = useCallback((direction: 1 | -1) => {
    setOpenIndex((prev) =>
      prev === null ? prev : (prev + direction + PROBLEMS.length) % PROBLEMS.length,
    );
  }, []);
